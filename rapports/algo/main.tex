...
Les figures suivantes illustrent les différences de rendu entre les algorithmes, qui seront étudiées plus en détail dans la section suivante.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/algorithmes_de_génération/représentations/default/fig_1.png}
        \caption{Algorithme PERFECT}
        \label{fig:triangle_1}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/algorithmes_de_génération/représentations/default/fig_2.png}
        \caption{Algorithme FUSION}
        \label{fig:triangle_2}
    \end{subfigure}

    \vspace{0.6cm}

    % Deuxième ligne : image centrée
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/algorithmes_de_génération/représentations/default/fig_3.png}
        \caption{Algorithme RANDOM (0.5)}
        \label{fig:triangle_3}
    \end{subfigure}

    \caption{Comparaison des labyrinthes générés selon différents algorithmes}
    \label{fig:labyrinthes_triangle}
\end{figure}
...
\info{Pour les labyrinthes aléatoires, la mesure enregistrée est la moyenne des résultats avec des pourcentages de 20\%, 30\%, 50\% (demande du sujet). Ce résultat est moyenné afin de présenter des modélisations uniformes.}
...
\obs{La distance paramétrée a un impact crucial puisqu'elle est maximale dans le cadre des labyrinthes aléatoires (spécification du sujet). Ainsi, l'algorithme doit trouver la position la plus éloignée (tenant compte des murs) en explorant tout le labyrinthe, le maximum étant inconnu à chaque génération.}
...
Sur la plage de tailles testées (de $5 \times 10$ à $500 \times 1000$), l’algorithme \texttt{PERFECT} issu du TP2 (parcours en profondeur) est systématiquement plus rapide que l’algorithme par fusion. Pour les grandes tailles, on observe qu’un labyrinthe généré par \texttt{PERFECT} est de l’ordre de dix fois plus rapide à produire qu’avec l’algorithme de fusion intégrant le calcul de distance.
